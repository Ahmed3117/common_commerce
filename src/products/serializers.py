...
class ProductSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Product
        fields = [
            'id', 'product_number','name','type','category','sub_category','brand', 'category_id', 'category_name', 'sub_category_id', 'sub_category_name',
...
class PillItemCreateUpdateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if not instance:
            user = self.context.get('effective_user') or self.context['request'].user

            if not getattr(user, 'is_authenticated', False) and not isinstance(user, User):
                return data
...
class FreeShippingOfferSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if target_type:
            target_fields = ['category', 'subcategory', 'brand']
