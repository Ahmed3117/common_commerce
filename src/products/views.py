...
class UserCartView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        return PillItem.objects.filter(user=user, status__isnull=True).order_by('-date_added')
...
class PillItemCreateView(generics.CreateAPIView):
    ...
    def get_serializer_context(self):
        ...
        context['effective_user'] = get_effective_user(self.request)
        return context
...
class PillItemDeleteView(generics.DestroyAPIView):
    ...
    def get_queryset(self):
        user = get_effective_user(self.request)
        return PillItem.objects.filter(user=user, status__isnull=True)
...
class PillCreateView(generics.CreateAPIView):
    ...
    def perform_create(self, serializer):
        user = get_effective_user(self.request)
        pill = serializer.save(user=user, status='i')
...
class PillCouponApplyView(generics.UpdateAPIView):
    ...
    def get_queryset(self):
        user = get_effective_user(self.request)
        return Pill.objects.filter(user=user)
...
class PillAddressCreateUpdateView(generics.CreateAPIView, generics.UpdateAPIView):
    ...
    def get_object(self):
        ...
        user = get_effective_user(self.request)
        pill = get_object_or_404(Pill, id=pill_id, user=user)
        try:
...
class PillDetailView(generics.RetrieveAPIView, PillItemPermissionMixin):
    ...
    def get_object(self):
        ...
        user = get_effective_user(self.request)
        return get_object_or_404(Pill, id=pill_id, user=user)
...
class UserPillsView(generics.ListAPIView):
    ...
    search_fields = ['pill_number', 'user__name', 'user__username', 'easypay_fawry_ref']

    def get_queryset(self):
        user = get_effective_user(self.request)
        return Pill.objects.filter(user=user).order_by('-date_added')
...
class LovedProductListCreateView(generics.ListCreateAPIView):
    ...
    def get_serializer_class(self):
        if self.request.method == 'POST':
            return AdminLovedProductSerializer
...
class PillItemRetrieveUpdateDestroyView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def perform_destroy(self, instance):
        if instance.pill and instance.pill.status in ['p', 'd']:
            raise ValidationError("Cannot delete items from paid/delivered pills")
        instance.delete()
    ...
    def delete(self, request, pill_id, item_id):
        ...
        try:
            ...
            removed_item_info = {
                ...
                'price': float(pill_item.price_at_sale) if pill_item.price_at_sale else 0.0
            }
...
class PillListCreateView(generics.ListCreateAPIView):
    ...
    filter_backends = [DjangoFilterBackend, rest_filters.SearchFilter]
    filterset_class = PillFilter
    search_fields = ['pilladdress__phone', 'pilladdress__government', 'pilladdress__name', 'user__name', 'user__username', 'pill_number', 'shakeout_invoice_id', 'shakeout_invoice_ref', 'easypay_invoice_uid', 'easypay_invoice_sequence', 'easypay_fawry_ref']
    pagination_class = CustomPageNumberPagination
    permission_classes = [IsAdminOrHasEndpointPermission]
