...
class Pill(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        # Track if paid/status changed only for existing DB rows
        if not is_new and self.pk:
            old_pill = Pill.objects.filter(pk=self.pk).first()

        is_newly_paid = bool(old_pill and not old_pill.paid and self.paid)
        if is_new and self.paid:
            is_newly_paid = True

        old_status = old_pill.status if old_pill else None
...
class FreeShippingOffer(models.Model):
    """
    Model for managing free shipping offers on products based on category, subcategory, or brand.
    """
    TARGET_TYPE_CHOICES = [
        ('category', 'Category'),
...
def prepare_whatsapp_message(phone_number, pill):
    ...
    customer_name = pill.user.username if pill.user else "عميلنا"
    message = (
